 ...
 * ----------------------------------------------------------------------------
 * CONFIG: the contact/quote forms insert a lead row directly into a Supabase
 * `leads` table via the REST API using the ANON key.
 *
 * Email notifications are intentionally NOT wired here. A Supabase Edge
 * Function (under a separate Resend account) will send the notification.
 ...
 * ------------------------------------------------------------------------- */
var CONFIG = {
  SUPABASE_URL:      'https://whduvsgyrbhmknurgkpo.supabase.co',
  SUPABASE_ANON_KEY: 'sb_publishable_oFZmZx8-EyJ68d2VtESrMg_EauSabUQ',
  LEADS_TABLE:       'leads'
};
...
(function () {
  ...
  /* ---- Lead forms -> Supabase leads insert (contact page + homepage) ---- */
  function bindLeadForm(form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      /* Validation (the forms use novalidate, so this is the only gate). */
      ...
      if (form.service && form.service.value !== '') {
